...
describe('assign', () => {
  it('should return { foo: 1 }', () => {
    expect(assign({ foo: 1 })).toEqual({ foo: 1 });
    expect(assign({ foo: 1 }, null)).toEqual({ foo: 1 });
  });

  it('should return { foo: 1, bar: 2 }', function () {
    expect(assign({ foo: 1 }, { bar: 2 })).toEqual({ foo: 1, bar: 2 });
  });

  it('should return { foo: 1, bar: 2, baz: 3 }', function () {
    expect(assign({ foo: 1 }, { bar: 2 }, { baz: 3 }))
      .toEqual({ foo: 1, bar: 2, baz: 3 });
  });

  it('should return { foo: 1, bar: 3, baz: 4 }', function () {
    expect(assign({ foo: 1, bar: 2 }, { bar: 3, baz: 4 }))
  ...
  it('should change target arguments and not change source arguments', function () {
    ...
    expect(assign(target, sourceOne, sourceTwo)).toEqual({ foo: 1, bar: 2, baz: 3 });
