...
describe('minMaxSum', () => {
  ...
  it('minMaxSum returns the sum of the smallest and largest numbers in an array', () => {
    ...
    expect(minMaxSum([10, 6, 23, 7, 12, -4, 0, -12])).toEqual(
      Math.min(10, 6, 23, 7, 12, -4, 0, -12) + Math.max(10, 6, 23, 7, 12, -4, 0, -12)
    );
    expect(minMaxSum([-1, -5, -20, -6, -9])).toEqual(
      Math.min(-1, -5, -20, -6, -9) + Math.max(-1, -5, -20, -6, -9)
    );
  });
});
