export const HashSHA = str => {
  ...
  for (var eyee = 0; eyee < str.length; eyee += 32) {
    a = H[0];
    b = H[1];
    c = H[2];
    d = H[3];
    ...
    for (var jay = 0; jay < 80; jay++) {
      if (jay < 16) {
        W[jay] = new int64(str[jay * 2 + eyee], str[jay * 2 + eyee + 1]);
      } else {
        W[jay] = safe_add_4(
          gamma1(W[jay - 2]),
          W[jay - 7],
      ...
      T1 = safe_add_5(h, sigma1(e), ch(e, f, g), K[jay], W[jay]);
  ...
  var binarray = [];
  for (var binArrayTemp = 0; binArrayTemp < H.length; binArrayTemp++) {
    binarray.push(H[binArrayTemp].highOrder);
    binarray.push(H[binArrayTemp].lowOrder);
  }
